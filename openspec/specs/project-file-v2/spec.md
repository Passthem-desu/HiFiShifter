...
工程文件格式从 JSON 升级为 MessagePack 二进制格式，版本号升至 v2，并新增媒体文件注册表。
...
**Requirement text:** 工程文件保存格式从 JSON 升级为 MessagePack，`ProjectFile` 版本号升至 v2。
...
**Requirement text:** `ProjectFile` 新增 `media_registry` 字段，记录工程引用的媒体文件信息。
