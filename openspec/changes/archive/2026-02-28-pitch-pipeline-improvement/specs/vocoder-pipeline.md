...
新建 `vocoder_pipeline.rs`，定义 `VocoderPipeline` trait，将声码器实现与调用方解耦。
...
`pitch_editing.rs` 中的 `maybe_apply_pitch_edit_to_clip_segment` 改为通过 `VocoderPipeline` trait 调用声码器，移除直接 match 分支。
