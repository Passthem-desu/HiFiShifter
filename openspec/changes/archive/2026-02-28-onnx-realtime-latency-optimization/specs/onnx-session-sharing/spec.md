...
ORT Session SHALL 在进程级别以 `OnceLock<Arc<Session>>` 形式全局共享，所有线程复用同一 Session 实例，消除每线程冷启动开销。
...
#### Scenario: Session 初始化失败时行为不变
- **WHEN** ONNX 模型文件不存在或 ORT 初始化失败
- **THEN** `is_available()` 返回 `false`，`infer_pitch_edit_mono()` 返回 `Err`，行为与现有实现一致
