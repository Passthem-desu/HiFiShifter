...
- **裁剪/伸缩范围**：拖动剪辑左右边界进行裁剪或延长；当剪辑长度超出源音频可用范围时会进入“循环重复”区间（波形预览也会重复显示，并在进入重复处出现竖线标记）。
...
- **内部偏移（Slip-Edit）**：按住 `Alt` + 鼠标左键拖动剪辑主体，可调整剪辑内部偏移（等价于修改 Trim In），不改变剪辑在时间线上的位置（不吸附）。
- **Slip-Edit 与重复段**：当剪辑进入“循环重复（Repeat）”区间时，Slip-Edit 会保持循环窗口长度不变，并支持在循环窗口内连续偏移（避免重复段与原段相位错乱/重叠）。
